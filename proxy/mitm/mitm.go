...
func (hw *HandlerWrapper) DumpHTTPAndHTTPs(resp http.ResponseWriter, req *http.Request) {
	...
	if !hw.https {
		...
		if err != nil {
			log.Println("dial to", host, "error:", err)
			return
		}
		...
		if err = req.Write(connOut); err != nil {
			log.Println("tls dial to", host, "error:", err)
			return
		}
		if err = req.Write(connOut); err != nil {
			log.Println("send to server error", err)
	...
	if err != nil {
		log.Println("respDump error:", err)
	}

	_, err = connIn.Write(respDump)
	if err != nil {
		log.Println("connIn write error:", err)
	}
...
func (hw *HandlerWrapper) Forward(resp http.ResponseWriter, req *http.Request, raddr string) {
	...
	if err != nil {
		log.Println("connectProxyServer error:", err)
	}
